 ...
 */
final class Version20210930063419 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('create definer = root@localhost procedure AutoUpdateEndedEventHideMe() no sql UPDATE event SET status_id = 7 WHERE status_id IN (5) AND DATE_ADD(DATE_ADD(start_date, INTERVAL duration MINUTE), INTERVAL 28 DAY) < CURRENT_TIMESTAMP()');
        $this->addSql('create definer = root@localhost procedure AutoUpdateNonPublishedAndCancelledEventHideMe() no sql UPDATE event SET status_id = 7 WHERE status_id IN (1, 6) AND DATE_ADD(date_created, INTERVAL 28 DAY) < CURRENT_TIMESTAMP()');
        ...
        $this->addSql('create definer = root@localhost event CallAutoUpdateEndedEventHideMe on schedule every \'30\' SECOND starts \'2021-09-29 12:48:36\' enable do call AutoUpdateEndedEventHideMe()');
        $this->addSql('create definer = root@localhost event CallAutoUpdateNonPublishedEventHideMe on schedule every \'30\' SECOND starts \'2021-09-29 12:48:36\' enable do call AutoUpdateNonPublishedEventHideMe()');
