...
final class Version20210930063419 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('create definer = root@localhost procedure AutoUpdateEndedEventHideMe() no sql UPDATE event SET status_id = 7 WHERE status_id IN (6) AND DATE_ADD(DATE_ADD(start_date, INTERVAL duration MINUTE), INTERVAL 28 DAY) < CURRENT_TIMESTAMP()');
        $this->addSql('create definer = root@localhost procedure AutoUpdateNonPublishedEventHideMe() no sql UPDATE event SET status_id = 7 WHERE status_id IN (1) AND DATE_ADD(date_created, INTERVAL 28 DAY) < CURRENT_TIMESTAMP()');
